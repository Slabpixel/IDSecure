/**
 * Custom dropdown for Squarespace form block select fields inside .ent-contact_form.
...
(function () {
  ...
  var ENHANCED_ATTR = 'data-ent-custom-select';
  var ROOT_SELECTOR = '.ent-contact_form';
  var FIELD_SELECTOR = ROOT_SELECTOR + ' .form-item.field.select';
  ...
  function concealNativeSelect(select) {
    ...
    select.classList.add('ent-custom-select__native');
    select.tabIndex = -1;
  ...
  function concealHostExtras(host, select) {
    ...
    Array.prototype.forEach.call(host.children, function (child) {
      ...
      if (child.classList && child.classList.contains('ent-custom-select')) {
        return;
      }
  ...
  function closeSelect(root) {
    ...
    var trigger = root.querySelector('.ent-custom-select__trigger');
    var listWrap = root.querySelector('.ent-custom-select__list-wrap');
  ...
  function closeAll(except) {
    document
      .querySelectorAll('.ent-custom-select.' + OPEN_CLASS)
      .forEach(function (el) {
  ...
  function updateDisplay(root, select, value, label) {
    ...
    var valueEl = root.querySelector('.ent-custom-select__value');
    var trigger = root.querySelector('.ent-custom-select__trigger');
    ...
    root.querySelectorAll('.ent-custom-select__option').forEach(function (opt) {
      opt.setAttribute(
        'aria-selected',
  ...
  function bindCustomSelect(custom) {
    if (custom.getAttribute('data-ent-custom-select-bound')) return;
    custom.setAttribute('data-ent-custom-select-bound', 'true');

    var trigger = custom.querySelector('.ent-custom-select__trigger');
    var list = custom.querySelector('.ent-custom-select__list');
    ...
    trigger.addEventListener('keydown', function (event) {
      if (
        ...
        openSelect(custom);
        var first = list.querySelector('.ent-custom-select__option');
        if (first) first.focus();
      }
    });

    list.addEventListener('click', function (event) {
      var item = event.target.closest('.ent-custom-select__option');
      if (!item) return;
    ...
    list.addEventListener('keydown', function (event) {
      var items = Array.prototype.slice.call(
        list.querySelectorAll('.ent-custom-select__option')
      );
      ...
      if (event.key === 'Enter' || event.key === ' ') {
        event.preventDefault();
        if (current && current.classList.contains('ent-custom-select__option')) {
          selectOption(
            custom,
  ...
  function buildCustomSelect(select, host, fieldWrap) {
    ...
    var listId =
      (fieldId ? fieldId + '-listbox' : 'ent-select-') +
      Math.random().toString(36).slice(2);

    var custom = document.createElement('div');
    custom.className = 'ent-custom-select';

    var trigger = document.createElement('button');
    trigger.type = 'button';
    trigger.className = 'ent-custom-select__trigger';
    ...
    valueSpan.className = 'ent-custom-select__value is-placeholder';
    valueSpan.textContent = placeholder;

    var chevron = document.createElement('span');
    chevron.className = 'ent-custom-select__chevron';
    ...
    listWrap.className = 'ent-custom-select__list-wrap';
    listWrap.setAttribute('data-lenis-prevent', '');
    listWrap.hidden = true;

    var list = document.createElement('ul');
    list.className = 'ent-custom-select__list';
    list.id = listId;
    ...
    options.forEach(function (opt, index) {
      ...
      item.className = 'ent-custom-select__option';
  ...
  function syncSelectField(fieldWrap) {
    ...
    var custom = host.querySelector('.ent-custom-select');
  ...
  function syncEntCustomSelects(root) {
    root = root || document;
    root.querySelectorAll(FIELD_SELECTOR).forEach(syncSelectField);
  }
  ...
  function scheduleSync(root) {
    ...
    syncTimers.set(
      target,
      setTimeout(function () {
        syncEntCustomSelects(target);
      }, 0)
    );
  }

  function observeForms() {
    document.querySelectorAll(ROOT_SELECTOR).forEach(function (form) {
      if (form.getAttribute('data-ent-select-observed')) return;
      form.setAttribute('data-ent-select-observed', 'true');
  ...
  document.addEventListener('click', function (event) {
    if (!event.target.closest('.ent-custom-select')) {
      closeAll(null);
    }
  });

  document.addEventListener('DOMContentLoaded', function () {
    syncEntCustomSelects();
    observeForms();
  });
